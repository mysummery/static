...
$(function() {
	...
	if(document.referrer.indexOf('address.html') != -1) {
		...
		$('.total_money').html(localStorage.valstotal);
	}
...
$(document).on('click', '.addresses', function(e) {
	...
	localStorage.gsId = gsId; //规格id
	localStorage.price = price;//单价
	localStorage.ids = ids; //闪电购ID
	localStorage.valstotal = valstotal;//总价
	localStorage.num = $('#indent-sub-num-input').val(); //个数
	localStorage.colors = $('.querycolor').html(); //颜色
	localStorage.types = $('.querytype').html(); //规格
					...
					if(data.code == "40000") {
						...
						ajaxPostAsyncData(slectInfoByUid, {
							...
							if(data.code == "40000") {
								...
								window.location.href = "/static/wechat/src/index/shop/indent_handle/indent_pay/indent_pay.html?orderid=" + orderid + "&price=" + valstotal + "&token=" + token;
							}
						}, 'json');
...
function checkinput_zzjs(event) {
	...
	valstotal = Number(price) * $('#indent-sub-num-input').val(); //加上浮动价之后的总价格
	valstotal = Number(valstotal).toFixed(2);
	$('.total_money').html(valstotal);
