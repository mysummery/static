...
$(function() {
	...
	ajaxWebPostAsyncData(slectInfoByUid, {
		...
		if(data.code == "40000") {
			...
			window.usertype = resdata.usertype;
			window.vipstate = resdata.vipstate;
			var parentcode = resdata.parentcode;
			$(".tzchongzhi").click(function() {
				if(usertype == 1) {
					window.location.href = "/static/pc/src/self/wallet/recharge/new_recharge.html?usertype=" + usertype + "&parentcode=" + parentcode + "";
				}
			})
		}
	}, 'json')
...
function transferRecord(){
	if(vipstate == '1') { 
		$.jBox.tip("您是普通学员,无法进行转账功能！", "error");
	}else {
		window.location.href = "/static/pc/src/self/wallet/bankTransfer/new_bankTransfer.html";
	}
}
function withdraw() {
	var uid = getCookie('cookie_brsy_pc_uid');
	if(vipstate == '1') { 
		$.jBox.tip("您是普通学员,无法进行提现功能！", "error");
	}else {
		...
		if(data.code == "40000") {
	        if (data.resobj.length == 0 || data.resobj == null){
	        	$.jBox.tip("您是还未绑定银行卡,请先进行绑定", "error");
	        }else {
	        	window.location.href = "/static/pc/src/self/wallet/bankWithdraw/new_bankWithdraw.html";
	        }
