...
function loadData() {
	...
	ajaxWebPostAsyncData(selectMyTuanduiAll, {
		...
		if(data.code == "40000") {
			//console.log(data);
			//alert(data.resobj.jifen);
			//alert(data.resobj.totalmoney);
...
function listSortBy(arr, field, order) {
	...
	$.each(result, function(index, value) {
		...
		var htmlstr = "<li onclick=\"team2('" + uid + "','"  + result[index].usercode + "','" + result[index].id + "','" + result[index].username + "','" + result[index].childcount + "')\">";
		htmlstr += "<div>";
...
function team2(uid,childusercode, childid, name, childcount) {
	window.location.href = "new_teammanager2.html?uid=" + uid + "&childusercode=" + childusercode +"&childid=" + childid + "&name=" + name + "&childcount=" + childcount;
}
loadData();
