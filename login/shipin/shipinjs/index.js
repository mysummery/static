...
//			点击侧边栏跳转
			$(".banner-nav li").click(function (){
					var idx = $(this).index();
				window.location.href = resdata[idx].pclinkurl;
				})
	...
	ajaxWebPostAsyncData(selectBykcid, {
		...
		if(data.code == "40000") {
			...
			$.each(resdata, function(index, value) {
				...
				if(index < 8) {
					$(".team-pic>ul").append('<li name=' + resdata[index].id + '><a class="tittle" href="new_chbofang.html?videoId=' + resdata[index].id + '&videoCourseId='+resdata[index].courseid +'&freestate=' + freestate + '"><img class="pic" src="' + resdata[index].pcimgurl + '"/><p>' + resdata[index].name + '</p></a><div class="click"><span class="arr"></span><span class="num">' + resdata[index].playcount + '</span></div></li>');
				}
